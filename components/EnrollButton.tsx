...
function EnrollButton({ courseId, isEnrolled }: { courseId: string; isEnrolled: boolean }) {
  ...
  const handleEnroll = async () => {
    ...
    startTransition(async () => {
      try {
        const { url } = await createStripeCheckout(courseId, user.id);
  ...
  return (
    <button
      className={`w-full rounded-lg px-6 py-3 font-medium transition-all duration-300 ease-in-out relative h-12
        ${!user?.id || isPending ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-white text-black hover:scale-105 hover:shadow-lg hover:shadow-black/10"}`}
